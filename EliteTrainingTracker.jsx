...
const computePercent = (date, fixedPercent) => {
  if (fixedPercent !== undefined) return fixedPercent;
...
const computeWorkoutForDate = (date) => {
  ...
  const toBlockItems = (blockName, items) => items.map((text, index) => ({ id: `${blockName}-${index}`, text }));
...
root.render(<EliteTrainingTracker />);
